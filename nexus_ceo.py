...
CASH_RESERVE: {f'KRITIK DUSUK {acc["musait"]/acc["toplam"]*100:.1f}% - Sadece pyramiding izinli!' if acc.get('toplam',1) > 0 and acc.get('musait',0)/acc.get('toplam',1) < 0.30 else f'OK {acc["musait"]/acc["toplam"]*100:.1f}%' if acc.get('toplam',1) > 0 else 'N/A'}
...
def execute_nexus_trade(analysis):
    ...
    for sym, side, size, sl, tp in matches:
        ...
        # CASH-RESERVE CHECK: Nur bei NEUEN Positionen (kein Pyramiding)
        # Bei Pyramiding (epic_positions nicht leer) wird dieser Block übersprungen
        if not epic_positions:
            try:
                acc = get_account_info(h)
                musait  = float(acc.get('musait', 0))   # verfuegbares Cash
                toplam  = float(acc.get('toplam', 0))   # Gesamtwert Depot
                if toplam > 0:
                    reserve_pct = musait / toplam
                    if reserve_pct < 0.30:
                        warnung = (f"{sym} BLOCKIERT: Cash-Reserve zu niedrig! "
                                   f"Verfuegbar: {musait:.2f} EUR = {reserve_pct*100:.1f}% "
                                   f"(Min: 30% von {toplam:.2f} EUR). "
                                   f"Nur Pyramiding bestehender Positionen erlaubt.")
                        results.append(warnung)
                        logging.warning(f"CASH-RESERVE BLOCK: {sym} - {reserve_pct*100:.1f}%")
                        continue
            except Exception as e:
                logging.error(f"Cash-Reserve Check Fehler: {e}")
