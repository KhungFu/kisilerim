...
PYRAMIDING ve EXIT KURALLARI:
- PYRAMIDING: 1,2,3,4 = Mevcut pozisyona EK ac. Eskilere DOKUNMA, TP laufen!
- PYRAMIDING: 0 = EXIT: TUM kapat + ayni mesajda karsit yon PYRAMIDING:1
- Trend devam ediyorsa PYRAMIDING: 0 YAZMA
...
def execute_nexus_trade(analysis):
    ...
    for sym, side, size, sl, tp in matches:
        ...
        if epic_positions:
            ...
            if is_gegenrichtung:
                pyra_match = re.search(
                    rf"TRADE:\s*{re.escape(sym)}\s*\|.*?PYRAMIDING:\s*(\d+)",
                    analysis, re.IGNORECASE | re.DOTALL)
                pyra_stufe = int(pyra_match.group(1)) if pyra_match else 1
                if pyra_stufe > 0:
                    results.append(f"{sym} Gegenrichtung ignoriert (PYRAMIDING:{pyra_stufe}) - TP laeuft")
                    continue
                geschlossen = 0
